 ...
 *
 * Program to solve the change problem using dynamic programming
 */
...
int main() {
  ...
  while ((rv = fgets(buf, MAX_LENGTH, stdin)) != NULL) {
    ...
    coins = malloc(sizeof(int) * num_coins);
    if (coins == NULL) {
    ...
    // Turn DEBUG to 1 to print the coin array
    if (DEBUG == 1) {
      i = 1;
